...
def PeopleWeCanMessage(request):
    people = Subscription.objects.filter(
        Q(subscriber=request.user) & Q(tier__can_message=True))
    ...
    if from_user != to_user:
        try:
            Message.objects.create(
                user=from_user, sender=from_user, body=body, recipient=to_user)
...
def UserSearch(request):
    ...
    if query:
        users = Subscription.objects.filter(
            Q(subscribed__username__icontains=query) & Q(tier__can_message=True))
